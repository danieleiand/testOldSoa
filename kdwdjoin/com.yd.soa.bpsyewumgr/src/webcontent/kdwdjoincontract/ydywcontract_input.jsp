		...
		function htlxchange(){ 
			var contracttype = $name("applyObject/htlx").value;    
			 if(contracttype == "3"){
				...
				$id("xkzj").style.display = "none";
			 	$id("xkzj").EOS_extendedAttribute=null;
          ...
          <w:panel id="panel_empinfo" titleStyle="font-weight: bold;color: black;" width="100%" title="员工基本信息（${orgtypeString}）">
				...
				<table align="center" border="0" width="100%" class="form_table" id="form_table2">
					...
					<tr id="xkzj">
						...
						<td colspan="1" width="30%">
							<d:select style="width:150px" dictTypeId="ISFZRSAMETOFR" property="applyObject/ifxkzj"  nullLabel="-请选择-" />
						</td>
						<td class="form_label">合规证件编号</td>
						<td colspan="1" width="40%">
							<h:text property="applyObject/xkzjmh" />
						</td>
					</tr>
				</table>
			</w:panel>
			...
			<w:panel id="panel6" width="100%" title="发起须知">
				<table align="center" border="0" width="100%" class="form_table" id="form_table2">
					...
					<tr>
				<td class="form_label"></td>
				<td colspan="3">网点加盟确认书模板下载：<a
					href="<%=request.getContextPath()%>/bpsyewu/kdwdjoincontract/jiamenqueren.pdf"
					style="color: blue;"><u>网点加盟确认书.pdf（点击下载）</u></a><font
					style="color: red">（此附件只适用合同类型为网点合作协议和特许加盟合同）</font>
					<a
					href="<%=request.getContextPath()%>/bpsyewu/kdwdjoincontract/danbaoqueren.docx"
					style="color: blue;"><u>担保确认书（自有公司为分公司担保20240218）.docx（点击下载）</u></a>
					</br>
				</td>
			</tr>
				</table>
        ...
        <script type="text/javascript">
          ...
          */
          function checkFormBeforeSubmit(){
			 ...
			 var ifxkzj = $name("applyObject/ifxkzj").value;
			 var xkzjmh = $name("applyObject/xkzjmh").value;
			 if(ifxkzj==1){
				if(!xkzjmh){
					alert("请填写许可证件编号");
					return false;
