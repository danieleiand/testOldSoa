...
package com.yd.soa.bpsyewumgr.jtsysj;
	 ...
	 */
	  ...
	  {  
		  JSONObject processData = new JSONObject();
			...
			String ywfl = processInfo.get("ywfl").toString();//1:有滴爱上城，2：欢猫
			String zt = processInfo.get("zt").toString();
			String fqf = processInfo.get("fqf").toString();
			String jsfqyname = processInfo.get("jsfqyname").toString();
			String jsfhandlername = processInfo.get("jsfhandlername").toString();
			String jsfhandlermobileno = processInfo.get("jsfhandlermobileno").toString();
			...
			processData.put("ywfl", ywfl);   //1:优递爱商城，2：欢猫
			processData.put("zt", zt);
			processData.put("fqf", fqf);
			processData.put("jsfqyname", jsfqyname);
			processData.put("jsfhandlername", jsfhandlername);
			processData.put("jsfhandlermobileno", jsfhandlermobileno);
			//活动定义id
			...
			//商业事业部负责人节点信息
			JSONObject sysybfzrData = new JSONObject();
//			if("1".equals(ywfl)){
//				//优递爱商城
				String sysybsyuserid = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("sysyb", "syuserid");
				sysybfzrData.put("sysybsyuserid", sysybsyuserid);
				String sysybsyusername = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("sysyb", "syusername");
				sysybfzrData.put("sysybsyusername", sysybsyusername);
				String sysybsymobile = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("sysyb", "symobile");
				sysybfzrData.put("sysybsymobile", sysybsymobile);
//			}else{
//				//欢猫
//				String sysybhmuserid = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("sysyb", "hmuserid");
//				sysybfzrData.put("sysybhmuserid", sysybhmuserid);
//				String sysybhmusername = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("sysyb", "hmusername");
//				sysybfzrData.put("sysybhmusername", sysybhmusername);
//				String sysybhmmobile = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("sysyb", "hmmobile");
//				sysybfzrData.put("sysybhmmobile", sysybhmmobile);
//			}
		    ...
		    System.out.println("商城流程调用韵签宝签字盖章--------------begin ");
		    LogUtil.logInfo("YD_INTF", "商城流程调用韵签宝签字盖章--------------begin ", null);
		    JSONObject postData = new JSONObject();
		    try
		    {
		      postData.put("processInfo", processData);
		      postData.put("fileInfo", fileData);
		      postData.put("sysybfzrNodeInfo", sysybfzrData);
		      postData.put("mscNodeInfo", mscData);
		      System.out.println("postData---"+postData.toJSONString());
		      postUrl = com.eos.foundation.eoscommon.BusinessDictUtil.getDictName("jtsysjcallyqb", "url");
		      System.out.println(postUrl);
//		      postUrl = "http://10.19.160.151:3333/yqb2soa/commplete";
	        
		      LogUtil.logInfo("YD_INTF", "商城流程调用韵签宝签字盖章{0}:返回消息{1}:【{2}】", null, processinstid,retData,postData.toJSONString());
		      ...
		      System.out.println("商城流程调韵签宝返回信息====="+jo.toJSONString());
		    ...
		    System.out.println("商城流程调用韵签宝签字盖章结束--------------end ");
		    System.out.println(retData);
		    LogUtil.logInfo("YD_INTF", "商城流程调用韵签宝签字盖章结束--------------end ", null);
		    return retData;
		  }
