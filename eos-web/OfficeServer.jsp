        ...
        if (DbaObj.OpenConnection()) {
          try {
            ...
            if (result.next()) {
              ...
              mTemplateId = result.getLong("TemplateId");
            }
            else {
              ...
              mTemplateId = (long)com.eos.foundation.database.DatabaseExt.getNextSequence("TemplateFile.templateid");
            }
            result.close();
          }
          catch (SQLException e) {
    ...
    private boolean SaveVersion() {
      ...
      try {
        ...
        //iFileId = DbaObj.GetMaxID("Version_File", "FileId");
        	iFileId = (long)com.eos.foundation.database.DatabaseExt.getNextSequence("VersionFile.fileid");
      		}else {
        		iFileId = Integer.getInteger(mFileID.toString()).longValue();
      		}
      
            prestmt = DbaObj.Conn.prepareStatement(Sql);
            prestmt.setLong(1, iFileId);
            prestmt.setString(2, mRecordID);
            prestmt.setString(3, mFileName);
            prestmt.setString(4, mFileType);
            prestmt.setInt(5, mFileSize);
        ...
        if (request.getMethod().equalsIgnoreCase("POST")) {
          ...
          if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) {         //判断是否是合法的信息包，或者数据包信息是否完整
            ...
            if (mOption.equalsIgnoreCase("LOADFILE")) {                           //下面的代码为打开服务器数据库里的文件
              ...
              if(mTemplate.contains("_VERSION_")){
                  System.out.println(mTemplate);
              	  mFileID = mTemplate.substring(9);
              	
              	  MsgObj.SetMsgByName("FILEID", mFileID);
              	  MsgObj.SetMsgByName("OPTION", "LOADVERSION");
              	  mOption="LOADVERSION";              	
              	  mRecordID = MsgObj.GetMsgByName("RECORDID");                        //取得文档编号
