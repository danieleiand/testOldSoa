...
var __system_menu= 
{
    ...
    {
      srcElement.className="txtcssonmouseover";
      //alert(1+srcElement.getElementsByTagName("img"))
    }	
    else
       if(srcElement.tagName.toLowerCase()=="img")
    {
      srcElement.parentNode.className="txtcssonmouseover";
      srcElement.icon=srcElement.src;
      srcElement.src=srcElement.overicon;
      //alert(1+srcElement.getElementsByTagName("img"))
    }
	},
	unactive:function()
    ...
    if(srcElement.tagName.toLowerCase()=="td")
    {
      srcElement.className="txtcss"
    }
        else
       if(srcElement.tagName.toLowerCase()=="img")
    {
      srcElement.parentNode.className="txtcss";
      srcElement.src=srcElement.icon;
      //alert(1+srcElement.getElementsByTagName("img"))
    }
	},

openPage : function (systemId)
   {
...
//3:切换树菜单
   ...
   if(menuInfo.logintype!= 1)
	   {
	    ...
	    openUrl=getAction(menuInfo);
		   ...
		   {
		   		openUrl ="<%=contextPath %>/portal/simlogin.jsp"; //用户映射的jsp页面
   ...
   var dataUrl=menuInfo.treeDataUrl
   window.top.portalUtil.showTree('mtree',menuInfo);
   
   //window.top.portalUtil.showTree('mtree',menuInfo.id,dataUrl,menuInfo.treeAsync,menuInfo.treeOpenmode,menuInfo.treeRootName,menuInfo.treeShowRoot)
...
Ext.util.CSS.createStyleSheet('.txtcssonmouseover{font-family: !important;font-size:9pt;color:#ff0000;}.txtcss{font-family:!important;font-size:9pt !important;color:#0B5877;}',"systemmenu");
</script>
...
.txtcss{font-size:9pt;color:#0B5877;}
	...
	<div   class="portal_widget_html" style="overflow-x:hidden" oncontextmenu="return false;">  	
	
	<Table width="100%" onmouseover="javascript:if(window.__system_menu)__system_menu.active()" onmouseout="if(window.__system_menu)__system_menu.unactive()" align="center" style="font-family:宋体 !important;font-size:9pt;color:#000000;">
<%
	//定义分列的变量，可以灵活更改的
	int col=3;
	...
	int i=0;
        for (Application sysmenu:sysmenus){	
			...
			if(!icon.startsWith("http"))icon=contextPath+icon;
			
			//取鼠标经过的图标
	
		int pos= icon.lastIndexOf(".");
        String  icon_onmouseover= icon.substring(0,pos)+"_mouseover"+icon.substring(pos);
	...
	strBuff.append("%' align='center' valign='middle' ><img style='cursor:hand'onclick=\"javascript:__system_menu.openPage('");
	strBuff.append(systemId);
	strBuff.append("')\" src='");
	strBuff.append(icon);
	strBuff.append("' overicon='"+icon_onmouseover+"' ><BR>");	
